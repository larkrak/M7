...
namespace proven\sequencies {
  ...
  define ( 'proven\sequencies\DNA_RNA_NUCLEOTIDES', 'acgtu' );
  define ( 'proven\sequencies\AMINOACIDS', 'ARNDCQEGHILKMFPSTWYV' );
  define ( 'proven\sequencies\DNA_NUCLEOTIDES_COMPL', array(
            'a' => 't',
            'c' => 'g',
            'g' => 'c',
            't' => 'a'
  ...
  */
  function generateRandomSequence( string $acceptedValues, int $length ): string {
     $seq = "";
     for ($i=0; $i<$length; $i++) {
  ...
  */
  function countOccurrencies( string $data, string $acceptedValues ): array {
